...
public class TriangleMesh extends SceneObject {
   ...
   public TriangleMesh(final File modelLocation, final ColorInformation colorInfo) {
      ...
      try {
         ...
         setBoundingVolume(new KDTree(this, vertices, normals, indices, colorInfo));
   ...
   private void readBinary(final File modelLocation, final int propertyCount, final int[] pos, final int[] normal, final boolean hasNormals) throws IOException {
      ...
      for (int i = 0; i < numVertices; i++) {
         ...
         vertices[i][0] = hasNormals ? properties[normal[0]] : 0;
         vertices[i][1] = hasNormals ? properties[normal[1]] : 0;
         vertices[i][2] = hasNormals ? properties[normal[2]] : 0;
      }
